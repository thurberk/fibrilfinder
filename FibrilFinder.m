...
answer=inputdlg('Note: if desired, start a Relion manual pick job & know how many computer cores you have before starting this program (enter anything)'); 
...
temp = textscan(fileID, 'resize_factor = %d %*[^\n]', 1);
resize_factor = temp{1};
temp = textscan(fileID, 'boxsize = %d %*[^\n]', 1);
boxsize = temp{1};
temp = textscan(fileID, 'winSize = %d %*[^\n]', 1);
winSize = temp{1};
temp = textscan(fileID, 'convlength = %d %*[^\n]', 1);
convlength = temp{1};
temp = textscan(fileID, 'grid_edge = %f %*[^\n]', 1);
grid_edge = temp{1};
temp = textscan(fileID, 'ignore_sm = %d %*[^\n]', 1);
ignore_sm = temp{1};
temp = textscan(fileID, 'T = %f %*[^\n]', 1);
T = temp{1};
fclose(fileID);

if ((resize_factor==0) || (boxsize==0) || (winSize==0) || (convlength==0) || (grid_edge==0) || (ignore_sm==0) || (T==0))
    error ('Input parameter read problem.  At least one value still 0');
end

answer=inputdlg('Read micrographs from CtfFind job micrographs_ctf.star file (enter 0) or list in Matlab (enter 1)'); 
...
for a=1:mics
    ...
    miclist2{a} = eraseBetween(miclist{a},"MotionCorr","icrographs/",'Boundaries','inclusive');
...
if (cores==1)
    for m=1:mics
        ...
        image_proc(Img_raw,resize_factor,grid_edge,winSize,convlength,ignore_sm,boxsize,T,relion_file,output_file,mout);
    end
else
