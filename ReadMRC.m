...
[maptemp,dcnt]=fread(f,nDataToRead,string);

if (s.mode==12)  % 16 bit float mode
    % is this double conversion necessary?
    % converts to half, then single
    map = single(half.typecast(maptemp));
else
    map = maptemp;
end
